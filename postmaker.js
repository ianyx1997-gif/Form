...
(function() {
  ...
  function generatePostImage(tour, callback) {
    ...
    img.onload = function() {
      ...
      var gradTop = ctx.createLinearGradient(0, 0, 0, H * 0.35);
      gradTop.addColorStop(0, 'rgba(0,0,0,0.5)');
      gradTop.addColorStop(1, 'rgba(0,0,0,0)');
      ctx.fillStyle = gradTop;
      ctx.fillRect(0, 0, W, H * 0.35);

      // Hotel name — center middle
      ...
      var fontSize = 52;
      if (hotelName.length > 30) fontSize = 40;
      if (hotelName.length > 45) fontSize = 34;
      ...
      // Draw hotel name lines centered vertically
      var lineHeight = fontSize * 1.3;
      var totalTextHeight = lines.length * lineHeight;
      var startY = (H / 2) - (totalTextHeight / 2) + fontSize / 2;
      for (var li = 0; li < lines.length; li++) {
        ctx.fillText(lines[li], W / 2, startY + li * lineHeight);
      }

      // Stars below hotel name
      ...
      if (tour.food) {
        ...
        ctx.fillText('🍽️ ' + tour.food, W - 40, bottomY);
      }
      ...
      if (tour.rating) {
        ...
        ctx.shadowBlur = 10;
        ctx.fillText('⭐ ' + tour.rating, 40, H - 40);
      }
    ...
    img.onerror = function() {
      ...
      ctx.fillText(tour.name || 'Hotel', W / 2, H / 2);
      if (tour.price) {
        ctx.font = 'bold 36px Arial';
        ctx.fillText(tour.price, W / 2, H / 2 + 60);
      }
      callback(canvas);
    };

    // Use image from tour data
    var imgSrc = tour.img || '';
