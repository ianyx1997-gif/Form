...
(function() {
  ...
  function drawTextOverlays(ctx, tour, W, H) {
    ...
    var fontSize = 68;
    if (hotelName.length > 30) fontSize = 56;
    if (hotelName.length > 45) fontSize = 46;
    ...
    ctx.fillText('📞 078 326 222', 40, H - 40);

    // Bottom right: date + price (shifted more left, more space between)
    ctx.textAlign = 'right';
    ctx.fillStyle = '#ffffff';
    var bottomY = H - 40;
    ...
    if (tour.price) {
      ...
      ctx.fillText(tour.price, W - 80, bottomY);
      bottomY -= 80;
    }
    if (tour.dates) {
      ctx.font = 'bold 28px Arial, Helvetica, sans-serif';
      ctx.fillText('📅 ' + tour.dates, W - 80, bottomY);
    }
