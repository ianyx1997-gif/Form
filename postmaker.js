...
(function() {
  ...
  // ===== CORS PROXY: fetch image as local blob =====
  var CORS_PROXIES = [
    function(u) { return 'https://corsproxy.io/?' + encodeURIComponent(u); },
    function(u) { return 'https://api.allorigins.win/raw?url=' + encodeURIComponent(u); }
  ];

  function fetchImageAsBlob(url, callback, proxyIndex) {
    if (typeof proxyIndex === 'undefined') proxyIndex = -1; // -1 = try direct first

    var fetchUrl = proxyIndex < 0 ? url : CORS_PROXIES[proxyIndex](url);
    ...
    fetch(fetchUrl, { mode: 'cors' })
      ...
      .catch(function() {
        // Try next proxy
        var next = proxyIndex + 1;
        if (next < CORS_PROXIES.length) {
          fetchImageAsBlob(url, callback, next);
        } else {
  ...
  function drawTextOverlays(ctx, tour, W, H) {
    ...
    if (tour.rating) {
      ...
      ctx.fillText('⭐ ' + tour.rating, W - 40, 55);
    }
    ...
    var fontSize = 54;
    if (hotelName.length > 30) fontSize = 44;
    if (hotelName.length > 45) fontSize = 36;
    ...
    if (tour.stars && tour.stars > 0) {
      ...
      ctx.fillText(starsText, W / 2, startY + lines.length * lineHeight + 10);
    }
    ...
    if (tour.food) {
      ...
      var foodY = startY + lines.length * lineHeight + (tour.stars ? 45 : 10);
    ...
    if (tour.price) {
      ...
      ctx.fillText(tour.price, W - 40, bottomY);
      bottomY -= 62;
    }
    if (tour.dates) {
      ctx.font = 'bold 28px Arial, Helvetica, sans-serif';
      ctx.fillText('📅 ' + tour.dates, W - 40, bottomY);
    }
  ...
  function generateExportableCanvas(tour, callback) {
    var imgSrc = tour.img || '';
    ...
    fetchImageAsBlob(imgSrc, function(blobUrl) {
      ...
      if (blobUrl) {
        // Got a local blob URL — canvas will be clean (exportable)
        var img = new Image();
        img.onload = function() {
          // Cover-fit the image
          ...
          callback(canvas, true); // true = exportable
        };
        img.onerror = function() {
          URL.revokeObjectURL(blobUrl);
          drawFallbackBg(ctx, W, H);
          drawTextOverlays(ctx, tour, W, H);
          callback(canvas, true);
        };
        img.src = blobUrl;
      } else {
        // All proxies failed — draw gradient background
        drawFallbackBg(ctx, W, H);
        drawTextOverlays(ctx, tour, W, H);
        callback(canvas, true);
      }
  ...
  function buildPreviewCard(tour) {
    var imgSrc = tour.img || '';
    if (imgSrc.indexOf('/320x240/') > -1) {
      imgSrc = imgSrc.replace('/320x240/', '/1200x900/');
    }
