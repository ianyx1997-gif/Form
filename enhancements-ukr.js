(function() {
  ...
  function createResultsCounter() {
    ...
    function updateCount() {
      ...
      counter.innerHTML = '<span class="count-num">' + count + '</span> готелів знайдено';
    }
  ...
  function addExpertButton() {
    ...
    var geo = geoEl ? geoEl.textContent.trim().replace(/\s*Arată pe hartă\s*/i, '').replace(/\s*Показати на карті\s*/i, '').trim() : '';
    ...
    var msg = 'Вітаю! Мене цікавить цей тур:\n\n';
    msg += 'Готель: ' + hotelName + '\n';
    if (geo) msg += 'Розташування: ' + geo + '\n';
    if (price) msg += 'Ціна: ' + price + '\n';
    if (dates) msg += 'Дати: ' + dates + '\n';
    if (nightsEl) msg += 'Ночей: ' + nightsEl + '\n';
    if (food) msg += 'Харчування: ' + food + '\n';
    if (rating) msg += 'Рейтинг: ' + rating + '\n';
    msg += window.location.href + '\n';
    msg += '\nХотів би дізнатися більше деталей. Можете допомогти?';

    // Create the expert button
    var expertBtn = document.createElement('a');
    expertBtn.className = 'zebra-expert-btn';
    expertBtn.href = 'https://wa.me/380630318111?text=' + encodeURIComponent(msg);
    expertBtn.target = '_blank';
    expertBtn.rel = 'noopener';
    expertBtn.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" style="vertical-align: middle; margin-right: 6px;"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H5.2L4 17.2V4h16v12zm-3-5h-2V9h2v2zm-4 0h-2V9h2v2zm-4 0H7V9h2v2z"/></svg>Запитати експерта про цей готель';
  ...
  var WISHLIST_KEY = 'pengi_wishlist';
  ...
  function addWishlistHearts() {
    ...
    cards.forEach(function(wrap) {
      ...
      geo = geo.replace(/\s*Arată pe hartă\s*/i, '').replace(/\s*Показати на карті\s*/i, '').trim();
      ...
      heartBtn.title = 'Додати до обраного';
      ...
      heartBtn.addEventListener('click', function(e) {
        ...
        // Show toast
        showWishlistToast(added ? '❤️ Додано до обраного' : 'Видалено з обраного');
      });
  ...
  function createWishlistPanel() {
    ...
    header.innerHTML = '<span class="zebra-wishlist-title"><svg viewBox="0 0 24 24" width="20" height="20" fill="#ef4444"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg> Обране</span>';
  ...
  function renderWishlistPanel() {
    ...
    if (list.length === 0) {
      body.innerHTML = '<div class="zebra-wishlist-empty">' +
        ...
        '<p>У вас немає турів в обраному</p>' +
        '<span>Натисніть ❤️ щоб зберегти</span>' +
        '</div>';
      return;
    }
  ...
  function buildShareText(list) {
    var text = 'Мої обрані тури на Pengi Travel:\n\n';
    list.forEach(function(tour, i) {
      ...
      text += '\n';
      if (tour.geo) text += '   Місце: ' + tour.geo + '\n';
      if (tour.food) text += '   Харчування: ' + tour.food + '\n';
      if (tour.dates) text += '   Дати: ' + tour.dates + '\n';
      if (tour.price) text += '   Ціна: ' + tour.price + '\n';
      if (tour.rating) text += '   Рейтинг: ' + tour.rating + '\n';
      if (tour.link) text += '   ' + tour.link + '\n';
      text += '\n';
    });
    text += 'Відкрийте для себе на pengi.travel';
    return text;
  }
  ...
  function renderWishlistFooter(list) {
    ...
    var expertText = 'Вітаю! Я обрав декілька готелів і хотів би вашої допомоги у виборі найкращого варіанту:\n\n';
    list.forEach(function(tour, i) {
    ...
    expertText += '\nЯкий варіант порадите?';
    expertBtn.href = 'https://wa.me/380630318111?text=' + encodeURIComponent(expertText);
    expertBtn.target = '_blank';
    expertBtn.rel = 'noopener';
    expertBtn.innerHTML = '<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H5.2L4 17.2V4h16v12zm-3-5h-2V9h2v2zm-4 0h-2V9h2v2zm-4 0H7V9h2v2z"/></svg> Запитати експерта';
    footer.appendChild(expertBtn);

    // Share label
    var shareLabel = document.createElement('div');
    shareLabel.className = 'zebra-wishlist-share-label';
    shareLabel.textContent = 'Поділитися обраним:';
    ...
    copyBtn2.innerHTML = '<svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/></svg> Копіювати';
    copyBtn2.addEventListener('click', function() {
      var text = buildShareText(list);
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(function() {
          showWishlistToast('Скопійовано в буфер!');
        });
      } else {
    ...
    clearBtn.textContent = 'Видалити все';
  ...
  function addTourPageWishlist() {
    ...
    heartBtn.setAttribute('data-tour-id', tourId);
    heartBtn.title = 'Додати до обраного';
    heartBtn.innerHTML = '<svg viewBox="0 0 24 24" width="22" height="22"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg> <span>Обране</span>';
